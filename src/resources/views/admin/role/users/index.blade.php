...
@section('content')
    <div class="container">
        ...
        <h1 class="header-title mb-4">@lang('site::user.users') @lang('rbac::role.with') {{ $role->title }}</h1>

        @include('rbac::admin.alert')

        <div class=" border p-3 mb-2">

            <a href="{{ route('admin.roles.show', $role) }}" class="d-block d-sm-inline btn btn-secondary">
                <i class="fa fa-close"></i>
                <span>@lang('site::messages.cancel')</span>
            </a>
        </div>
        @filter(['repository' => $repository, 'route_param' => $role])@endfilter
